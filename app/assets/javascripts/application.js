...
$(document).ready(function() {
  ...
  $('select #survey').change(function(){
    ...
    var surveyId = $(survey).attr('survey-id');
  ...
  $('select').change(function(){
    if($(this).attr('name') === 'location'){
      ...
      $.ajax({
        ...
        // populate_surveys(data)
        $('select#survey').empty();
        for(i = 0; i < data.length; i++){
          $("select#survey").append(
              $("<option></option>").attr("survey-id", data[i].id).text(data[i].name)
            )
        }
      ...
      $.ajax({
        ...
        for(i = 0; i < data.length; i++){
          ...
          $("select#user").append(
              $("<option></option>").attr("user-id", data[i].id).text(name)
            )
        }
        $('select').material_select();
      })
